...
export function createCharacter(THREE, kind = 'kanshan') {
  ...
  let height = 1.7;
  let torso;
  const white = material('white', 0xf7f5f0, 0.15);
  const black = material('black', 0x17191d, 0.25);

  if (kind === 'kanshan') {
    // Official Liu Kanshan: one smooth white egg body, big black nose,
    // two dot eyes, white pointed ears, thin black limbs, white tail ball.
    // No scarf, no bag — those were invented and don't match the official art.
    torso = sphere(root, 1, white, 0, 1.05, 0, 0.62, 0.92, 0.52, 'torso');
    // Pointed white ears (rounded tips) on top of the head.
    cone(root, 0.16, 0.42, white, -0.28, 1.88, -0.02, 0, 0.12, 'ear-left');
    cone(root, 0.16, 0.42, white, 0.28, 1.88, -0.02, 0, -0.12, 'ear-right');
    // Big black round nose protruding from the face.
    sphere(root, 0.26, black, 0, 1.32, 0.50, 1.15, 0.9, 0.85, 'nose');
    // Two tiny dot eyes above and to the sides of the nose.
    eye(root, -0.20, 1.55, 0.42, 0.045); eye(root, 0.20, 1.55, 0.42, 0.045);
    // Thin black legs with round feet.
    const legMat = black;
    const lLeg = new THREE.Group(), rLeg = new THREE.Group();
    lLeg.position.set(-0.18, 0.55, 0); rLeg.position.set(0.18, 0.55, 0);
    root.add(lLeg, rLeg);
    cyl(lLeg, 0.07, 0.40, legMat, 0, -0.20, 0);
    cyl(rLeg, 0.07, 0.40, legMat, 0, -0.20, 0);
    sphere(lLeg, 0.12, legMat, 0, -0.40, 0.05, 1.2, 0.7, 1.3, 'foot');
    sphere(rLeg, 0.12, legMat, 0, -0.40, 0.05, 1.2, 0.7, 1.3, 'foot');
    limbs.legs.push(lLeg, rLeg);
    // Thin black arms hanging naturally at the sides.
    limbs.arms.push(limbGroup(root, -0.55, 1.25, 0.02, -1, black, 0.42, 0.06), limbGroup(root, 0.55, 1.25, 0.02, 1, black, 0.42, 0.06));
    // White fluffy tail ball on the back.
    sphere(root, 0.16, white, 0, 0.95, -0.50, 1, 1, 1, 'tail');
    height = 1.72;
  } else if (kind === 'bear') {
