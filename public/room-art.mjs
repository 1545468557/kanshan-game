...
function submitCase(){const scores=blueBloodCase.questions.map(q=>scoreAnswer(q,caseAnswers.get(q.id)||''));const total=scores.reduce((sum,value)=>sum+value,0);caseSubmitted=true;caseResult.hidden=false;caseResult.innerHTML=`<strong>本次解释：${total} / 5</strong><br>${total===5?'你把蓝血、来源、昨夜经过、撒谎者和动机全部串起来了。':total>=3?'大部分关键关系已经被你抓住，仍有几处需要重新核对。':'你的解释还很早期，房间里还有不少细节没有被重新组合。'}<details><summary>查看标准答案</summary>${blueBloodCase.questions.map((q,i)=>`<p><b>${i+1}. ${q.label}</b><br>${q.answer}<br><small>本题得分：${scores[i]} / 1</small></p>`).join('')}</details>`;
  // 静默把这次结案记到服务端，方便管理员回看。记录失败不影响玩家看到的分数。
  fetch('/api/submit',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({sessionId,answers:Object.fromEntries(caseAnswers)})}).catch(()=>{});
}
$('case-submit').onclick=submitCase;
...
// 玩家已经近看过的物件。它不会被送进模型当「事实」，只用来估计当事人此刻有多不安：
// 看得越多，问到他头上时语气越短、越防备。这样缓存能命中，台词又确实随调查进度变化。
const inspectedIds=new Set();
// 匿名会话编号：只写在本机浏览器里，用来把同一位访客的提问串起来。不含任何身份信息。
const sessionId=(()=>{try{const key='zhihu-blueblood-session',found=localStorage.getItem(key);if(found)return found;const made='s-'+Math.random().toString(36).slice(2,10)+Date.now().toString(36).slice(-4);localStorage.setItem(key,made);return made;}catch{return 's-anon';}})();
// 每个 NPC 自己的对话历史，作为追问时的上下文（只保留最近几轮）
const npcHistory=new Map();
let npcBusy=false;
function renderNPCs(id){
  $('npc-copy').innerHTML=blueBloodCase.npc.filter(person=>!id||person.id===id).map(person=>`<article class="npc-card"><h3>${person.name}</h3><p>${person.opening}</p><div class="npc-ask"><input aria-label="向${person.name}提问" placeholder="问一个具体问题……" data-npc="${person.id}"><button type="button" data-ask-npc="${person.id}">追问</button></div><p class="npc-answer" data-npc-answer="${person.id}" hidden></p><p class="npc-status" data-npc-status="${person.id}" aria-live="polite"></p></article>`).join('')+'<p class="tiny">每个人的说法都受他自己所知范围的限制，也会记住你刚才问过什么。</p>';
}
// 本地预案台词。模型连不上、超额、超时或回答不合角色时用它顶上，保证游戏永远能玩下去。
// 这段逻辑就是 v1 到 v39 期间唯一在用的对话实现，保留原文。
function npcReplyLocal(person,text){
  const t=text.toLowerCase();
  if(person.id==='trainer'){
    if(/假伤|贴片|道具|瓶|模拟/.test(t))return '教具里确实有模拟用品，但你不能仅凭它们证明昨晚使用过。';
    if(/录像|拍摄|反应|观察|测试|为什么/.test(t))return '没有什么额外安排。你当时比较紧张，别的都不能说明什么。';
    return person.opening;
  }
  if(person.id==='zhangwei'){
    if(/看见|血|颜色|伤口/.test(t))return '我看见的是蓝色液体，不是伤口内部。现在想想，我当时把“看到”直接当成了“理解”。';
    if(/坚持|聚餐|改口|培训师|老师/.test(t))return '他没跟我提过你。他平时就是了解一下培训对象，没别的。';
    return person.opening;
  }
...
// 问一句话：先走后端的大模型，拿不到就退回上面的本地预案。
// 界面永远有话说，是这里最重要的一条 —— 模型挂了不该让玩家卡在对话框里。
async function askNpc(person,question){
  const payload={npcId:person.id,question,observed:[...inspectedIds],history:npcHistory.get(person.id)||[],sessionId};
  try{
    const response=await fetch('/api/npc',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(payload)});
    const data=await response.json();
    if(data&&data.ok&&data.reply)return{reply:data.reply,source:data.source,note:''};
    return{reply:npcReplyLocal(person,question),source:'local',note:data&&data.message?data.message:''};
  }catch{
    return{reply:npcReplyLocal(person,question),source:'local',note:'连不上对话服务，已使用既定台词。'};
  }
}
function rememberTurn(person,question,reply){
  const list=npcHistory.get(person.id)||[];
  list.push({role:'user',content:question},{role:'assistant',content:reply});
  npcHistory.set(person.id,list.slice(-6));
}
const npcClose=$('npc-close');if(npcClose)npcClose.onclick=()=>npcBoard.close();
async function submitNpcQuestion(button){
  if(npcBusy)return;
  const person=blueBloodCase.npc.find(item=>item.id===button.dataset.askNpc);
  if(!person)return;
  const input=document.querySelector(`input[data-npc="${person.id}"]`);
  const answer=document.querySelector(`[data-npc-answer="${person.id}"]`);
  const status=document.querySelector(`[data-npc-status="${person.id}"]`);
  const question=input.value.trim();
  if(!question){input.focus();return;}
  npcBusy=true;button.disabled=true;input.disabled=true;
  status.dataset.state='thinking';status.textContent='他正在斟酌怎么回答……';
  const result=await askNpc(person,question);
  npcBusy=false;button.disabled=false;input.disabled=false;input.value='';
  answer.textContent=result.reply;answer.hidden=false;
  rememberTurn(person,question,result.reply);
  if(result.source==='local'){
    // 如实标注，不把「本地既定台词」说成「大模型即兴」。这一行也是你自测密钥是否配好的信号。
    status.dataset.state='local';
    status.textContent=result.note||'模型暂不可用，使用既定台词。';
  }else{
    status.dataset.state='ai';
    status.textContent=result.source==='cache'?'由大模型即兴扮演（同一句话沿用先前回答）':'由大模型即兴扮演';
  }
  input.focus();
}
$('npc-copy').addEventListener('click',e=>{const button=e.target.closest('[data-ask-npc]');if(button)submitNpcQuestion(button);});
$('npc-copy').addEventListener('keydown',e=>{
  if(e.key!=='Enter')return;
  const input=e.target.closest('input[data-npc]');if(!input)return;
  e.preventDefault();
  const button=document.querySelector(`[data-ask-npc="${input.dataset.npc}"]`);
  if(button)submitNpcQuestion(button);
});
